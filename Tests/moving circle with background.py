...
pygame.display.set_caption("Type-R Survivor")
...
# draw background function to be called in the game loop
def draw_bg():
    for x in range(5):
        speed = 1
        for i in bg_images:
...
# draw ground function to be called in the game loop
def draw_ground():
    for x in range(15):
    ...
    if not game_over_active:
        if not store_active:
            ...
            if keys[pygame.K_a] and scroll > 0: 
                move_vector.x -= 1
                scroll -= 5 # Scroll background left
            if keys[pygame.K_d] and scroll < 50000: 
                move_vector.x += 1
                scroll += 5 # Scroll background right
